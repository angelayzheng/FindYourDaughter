...
def evaluate(dataset: Path, tolerance_mm: float = 3.0) -> dict:
    ...
    for truth_path in truth_files:
        ...
        prediction = run_case(truth_path.with_name(f"orig{suffix}.nii"),
                              truth_path.with_name(f"mask{suffix}.nii"))["daughters"]
        matches = []
    ...
    return {"scope": "Generated tubes only; development tolerance, not official challenge scoring",
...
def main() -> int:
    ...
    result = evaluate(args.dataset, args.tolerance_mm)
